...
rt_inline void __s2_clear_pgd(pgd_t *pgd_ptr)
{
    WRITE_ONCE(*pgd_ptr, 0);
}

rt_inline void __s2_clear_pud(pud_t *pud_ptr)
{
    WRITE_ONCE(*pud_ptr, 0);
}

rt_inline void __s2_clear_pmd(pmd_t *pmd_ptr)
{
    WRITE_ONCE(*pmd_ptr, 0);
}

rt_inline void __s2_clear_pte(pte_t *pte_ptr)
{
    WRITE_ONCE(*pte_ptr, 0);
}

rt_inline void __s2_set_pgd(pte_t *pgd_ptr, pgd_t pgd_val)
{
    WRITE_ONCE(*pgd_ptr, pgd_val);
}

rt_inline void __s2_set_pud(pud_t *pud_ptr, pud_t pud_val)
{
    WRITE_ONCE(*pud_ptr, pud_val);
}

rt_inline void __s2_set_pmd(pmd_t *pmd_ptr, pmd_t pmd_val)
{
    WRITE_ONCE(*pmd_ptr, pmd_val);
}

rt_inline void __s2_set_pte(pte_t *pte_ptr, pte_t pte_val)
{
    WRITE_ONCE(*pte_ptr, pte_val);
}

static rt_err_t __s2_map_pmd_huge(pmd_t *pmd_ptr, struct mem_desc *desc)
{
    if ((*pmd_ptr))
        return -RT_ERROR;
    ...
    if (!IS_BLOCK_ALIGN(desc->vaddr_start) 
     || !IS_BLOCK_ALIGN(desc->paddr_start)
...
static rt_err_t __s2_map_pud_huge(pud_t *pud_ptr, struct mem_desc *desc)
{
    if ((*pud_ptr))
        return -RT_ERROR;
...
static void __s2_map_pte(pte_t *pte_tbl, struct mem_desc *desc)
{
    pte_t *pte_ptr;
    ...
    do
    {
        if (!(*pte_ptr))
        {
            __s2_set_pte(pte_ptr, desc->paddr_start | pte_attr);
...
static rt_err_t __s2_map_pmd(pmd_t *pmd_tbl, struct mem_desc *desc)
{
    pmd_t *pmd_ptr;
    pte_t *pte_tbl;
    ...
    do
    {
        ...
        if (__s2_map_pmd_huge(pmd_ptr, desc) == RT_EOK)
            __s2_set_pmd(pmd_ptr, desc->attr | (desc->paddr_start & S2_VA_MASK));
        else
        {
            if (*pmd_ptr)
            {
                /* get next level pte */
                pte_tbl = (pte_t *)(*pmd_ptr & NEXT_LEVEL_ADDR_MASK);
            }
            else
            {
                /* populate first pte */
                ...
                __s2_set_pmd(pmd_ptr, pte_attr | (*pte_tbl & NEXT_LEVEL_ADDR_MASK));
            }
        }

        __s2_map_pte(pte_tbl, desc);
...
static rt_err_t __s2_map_pud(pud_t *pud_tbl, struct mem_desc *desc)
{
    pud_t *pud_ptr;
    pmd_t *pmd_tbl;
    ...
    do
    {
        ...
        if (__s2_map_pud_huge(pud_ptr, desc) == RT_EOK)
            __s2_set_pud(pud_ptr, desc->attr | (desc->paddr_start & S2_VA_MASK));
        else
        {
            if (*pud_ptr)
            {
                /* get next level pmd */
                pmd_tbl = (pmd_t *)(*pud_ptr & NEXT_LEVEL_ADDR_MASK);
            }
            else
            {
                /* populate first pmd */
                ...
                rt_uint64_t pte_attr = MMU_TYPE_TABLE;
                __s2_set_pud(pud_ptr, pte_attr | (*pmd_tbl & NEXT_LEVEL_ADDR_MASK));
            }
        }

        __s2_map_pmd(pmd_tbl, desc);
    } while (pud_ptr++, desc->paddr_start += size, desc->vaddr_start = next, 
             desc->vaddr_start != desc->vaddr_end);

    return RT_EOK;    
}

static void __s2_unmap_pte(pte_t *pte_tbl, rt_ubase_t va, rt_ubase_t va_end)
{
    ...
    {   
        ...
        if (*pte_tmp)
        {
            __s2_clear_pte(pte_tmp);
            rt_free(pte_tmp);
        }
    } while (va += RT_MM_PAGE_SIZE, va != va_end);
}

static void __s2_unmap_pmd(pmd_t *pmd_tbl, rt_ubase_t va, rt_ubase_t va_end)
{
    ...
    do
    {
        ...
        if (*pmd_ptr)
        {
            ...
            {
                __s2_clear_pmd(pmd_ptr);
                rt_free(pmd_ptr);
            }
            else
            {
                pte_tbl = (pte_t *)(*pmd_ptr & NEXT_LEVEL_ADDR_MASK);
                __s2_unmap_pte(pte_tbl, va, next);
...
static rt_err_t __s2_unmap_pud(struct mm_struct *mm, rt_ubase_t va, rt_ubase_t va_end)
{
    ...
    do
    {
        ...
        {
            pmd_tbl = (pmd_t *)(*pud_ptr & NEXT_LEVEL_ADDR_MASK);
            __s2_unmap_pmd(pmd_tbl, va, next);
            if (((va & ~S2_PUD_MASK) == 0) && ((va_end - va) == S2_PUD_SIZE))
            {
                __s2_clear_pud(pud_ptr);
                rt_free(pud_ptr);
                rt_free(pmd_tbl);
            }
        }
...
static rt_err_t __s2_ipa2pa(struct mm_struct *mm, rt_uint64_t va, rt_ubase_t *pa)
{
...
{
    void *guest_pgd = rt_malloc_align(S2_PAGETABLE_SIZE, RT_MM_PAGE_SIZE);
    if (!guest_pgd)
    {
        rt_kprintf("[Error] Allocate memory for guest pgd failure.\n");
        return RT_NULL;
    }
    else
    {
        rt_memset(guest_pgd, 0, S2_PAGETABLE_SIZE);
        rt_kprintf("[Info] Guest pgd: 0x%08x - 0x%08x.\n", 
                    guest_pgd, guest_pgd + S2_PAGETABLE_SIZE);
        return guest_pgd;
    }
}
...
rt_err_t stage2_map(struct mm_struct *mm, struct mem_desc *desc)
{
    ...
    RT_ASSERT((desc->vaddr_start < S2_IPA_SIZE) 
           && (desc->vaddr_end < S2_IPA_SIZE));
    RT_ASSERT(desc->paddr_start < S2_PA_SIZE);
    RT_ASSERT(IS_PAGE_ALIGN(desc->vaddr_start) 
           && IS_PAGE_ALIGN(desc->vaddr_end) 
           && IS_PAGE_ALIGN(desc->paddr_start));

    return __s2_map_pud(mm->pgd_tbl, desc);    
}
...
rt_err_t stage2_unmap(struct mm_struct *mm, rt_ubase_t va, rt_ubase_t va_end)
{
	...
	return __s2_unmap_pud(mm, va, va_end);
}